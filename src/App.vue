<template>
  <div id="app" class="contain">
    <game-state-start v-if="uiState === 'start'">
      ...
      <p
        v-for="option in characterChoise"
        :key="option"
        class="character-choise"
      >
        <input type="radio" v-model="charInput" :id="option" :value="option" />
        <label :for="option">{{ option }}</label>
      </p>
      <button @click="pickCharcther">Pick your charchter</button>
    </game-state-start>
    <section v-else-if="uiState ==='characterChoisenn' ">
      ...
      <div class="zombietalk">
        <p v-for="character in shuffle(characterChoise)" :key="character">
...
export default {
  ...
  computed: {
    ...mapState(["uiState", "questions", "characterChoise", "character","questionIndex","score"]),
  },
  methods: {
    pickCharcther() {
      this.$store.commit("pickChar", this.charInput);
      this.$store.commit("updateUiState", "characterChoisenn");
    },
   pickQuestion(character){
     this.$store.commit("pickkkkkQuestion",character)
   },
   shuffle(array){
